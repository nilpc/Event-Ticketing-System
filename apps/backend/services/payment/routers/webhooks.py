...
def _get_webhook_service(session: AsyncSession=Depends(get_db_session)) -> WebhookService:
...
@router.post('/stripe')
async def stripe_webhook(request: Request, svc: WebhookService=Depends(_get_webhook_service)) -> dict:
