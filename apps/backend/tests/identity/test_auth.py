...
async def test_signup_returns_201(client: AsyncClient) -> None:
    ...
    r = await client.post('/v1/auth/signup', json={'email': email, 'password': 'Str0ng!Pass#2024'})
...
async def test_signup_invalid_email_returns_422(client: AsyncClient) -> None:
    r = await client.post('/v1/auth/signup', json={'email': 'not-an-email', 'password': 'Str0ng!Pass#2024'})
    assert r.status_code == 422
...
async def test_login_nonexistent_user_returns_401(client: AsyncClient) -> None:
    r = await client.post('/v1/auth/login', json={'email': f'nope_{uuid.uuid4().hex[:8]}@example.com', 'password': 'Str0ng!Pass#2024'})
    assert r.status_code == 401
...
async def test_protected_endpoint_accepts_valid_jwt(client: AsyncClient) -> None:
    ...
    r = await client.get('/v1/bookings', headers={'Authorization': f'Bearer {token}'})
...
async def test_protected_endpoint_rejects_invalid_jwt(client: AsyncClient) -> None:
    r = await client.get('/v1/bookings', headers={'Authorization': 'Bearer invalid.token.here'})
    assert r.status_code == 401
