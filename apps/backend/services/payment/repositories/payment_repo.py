...
class PaymentRepository:
    ...
    async def create_payment_record(
        self, payment_id: UUID, booking_id: UUID, amount: Decimal, status: str
    ) -> None:
        payment = Payment(
    ...
    async def update_payment_record(
        self, payment_id: UUID, provider_payment_id: str | None = None, status: str | None = None
    ) -> None:
        values: dict = {}
    ...
    async def get_active_payment_for_booking(self, booking_id: UUID) -> Payment | None:
        ...
        result = await self.session.execute(
            select(Payment).where(
                and_(Payment.booking_id == booking_id, ~Payment.status.in_(terminal_statuses))
            )
        )
