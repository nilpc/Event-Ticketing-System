...
async def _require_merchant(
    user_id: UUID = Depends(get_current_user_id), session: AsyncSession = Depends(get_db_session)
) -> User:
...
async def _assert_can_schedule(
    ...
    if event.created_by != merchant.user_id:
        raise HTTPException(
            status_code=403, detail="Cannot schedule showtimes for events created by others."
        )
    if venue.created_by != merchant.user_id:
        raise HTTPException(
            status_code=403, detail="Cannot schedule showtimes at venues created by others."
        )
...
async def update_showtime(
    ...
    if not merchant.is_master_admin:
        ...
        if owner != merchant.user_id:
            raise HTTPException(
                status_code=403, detail="Cannot modify showtimes for events created by others."
            )
    try:
...
async def delete_showtime(
    ...
    if not merchant.is_master_admin:
        ...
        if owner != merchant.user_id:
            raise HTTPException(
                status_code=403, detail="Cannot delete showtimes for events created by others."
            )
    try:
