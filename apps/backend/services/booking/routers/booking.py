...
@router.post(
    "/book", response_model=BookResponse, responses={409: {"model": dict}, 422: {"model": dict}}
)
async def book_seats(
    payload: BookRequest, request: Request, svc: BookingService = Depends(_get_booking_service)
) -> BookResponse:
...
async def mock_confirm(
    booking_id: UUID, svc: BookingService = Depends(_get_booking_service)
) -> MockConfirmResponse:
    try:
...
async def list_bookings(
    request: Request, svc: BookingService = Depends(_get_booking_service)
