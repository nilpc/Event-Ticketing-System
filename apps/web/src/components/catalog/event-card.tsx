...
export function EventCard({ event }: EventCardProps) {
  ...
  return (
    <motion.div variants={cardVariants}>
      <Card
        className="h-full group overflow-hidden hover:shadow-xl hover:shadow-primary/5 transition-all duration-500 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
        onClick={openCard}
        role="button"
        tabIndex={0}
        onKeyDown={(e) => {
      ...
      >
        ...
        <CardHeader className="pb-3">
          ...
          <CardTitle className="text-lg group-hover:text-primary transition-colors duration-300 flex items-center gap-2">
            ...
            {event.name}
            <ArrowUpRight className="h-4 w-4 text-primary ml-auto opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300" />
          </CardTitle>
        </CardHeader>
