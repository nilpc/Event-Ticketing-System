...
async def _seed_user(session: AsyncSession) -> uuid.UUID:
    user_id = uuid.uuid4()
    user = User(
        user_id=user_id, email=f"concurrency_{user_id.hex[:8]}@test.com", password_hash="dummy-hash"
    )
    session.add(user)
    await session.flush()
    return user_id
...
async def _attempt_booking(
    session_factory: async_sessionmaker, show_id: uuid.UUID, seat_id: str, user_id: uuid.UUID
) -> bool:
...
async def test_zero_double_bookings_under_concurrency() -> None:
    ...
    tasks = [
        _attempt_booking(async_session_factory, show_id, seat_ids[i % len(seat_ids)], user_ids[i])
...
async def test_seat_status_consistency() -> None:
    ...
    async with async_session_factory() as session:
        result = await session.execute(
            select(Seat.seat_id, Seat.status).where(Seat.show_id == show_id).order_by(Seat.seat_id)
        )
