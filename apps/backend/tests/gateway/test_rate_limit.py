...
def _low_rate_limits():
    ...
    def _create_memory_limiter() -> Limiter:
        return Limiter(key_func=get_remote_address, storage_uri='memory://', default_limits=[test_settings.RATE_LIMIT_PUBLIC], headers_enabled=True)
    _sm._find_route_handler = _find_route_handler
    with patch('core.config.settings', test_settings), patch('core.middleware.rate_limit.settings', test_settings), patch('core.middleware.rate_limit.create_limiter', side_effect=_create_memory_limiter):
        yield
...
class TestRedisOutageResilience:
    ...
    def _dead_redis_limiter(self):
        ...
        def _create_dead_limiter() -> Limiter:
            return Limiter(key_func=get_remote_address, storage_uri='redis://127.0.0.1:1/0', default_limits=[test_settings.RATE_LIMIT_PUBLIC], in_memory_fallback=[test_settings.RATE_LIMIT_PUBLIC], in_memory_fallback_enabled=True)
        _sm._find_route_handler = _find_route_handler
        with patch('core.config.settings', test_settings), patch('core.middleware.rate_limit.settings', test_settings), patch('core.middleware.rate_limit.create_limiter', side_effect=_create_dead_limiter):
            yield
    ...
    async def test_app_does_not_crash_when_redis_down(self, client: AsyncClient) -> None:
        for _ in range(3):
            r = await client.get('/v1/venues')
            assert r.status_code in (200, 404, 429), f'Expected survivable status, got {r.status_code}'
