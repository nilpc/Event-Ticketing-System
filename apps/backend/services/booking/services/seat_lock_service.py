...
class SeatLockService:
    ...
    def __init__(self, session: AsyncSession, lock_repo: LockRepository, seat_repo: SeatRepository) -> None:
    ...
    async def lock_seats(self, show_id: UUID, seat_ids: list[str], user_id: UUID) -> SeatLockResponse:
        ...
        try:
            for seat_id in seat_ids:
                if not await self.lock_repo.acquire_user_hold(show_id, user_id, HOLD_LIMIT_TTL, max_holds=MAX_SEATS_PER_CHECKOUT):
                    raise BookingConflictError('You have reached the maximum number of held seats. Please complete or release existing holds.')
                acquired = await self.lock_repo.acquire_seat_lock(show_id, seat_id, user_id, SEAT_LOCK_TTL)
                if not acquired:
        ...
        return SeatLockResponse(idempotency_key=idempotency_key, expires_at=expires_at, locked_seat_ids=seat_ids)
