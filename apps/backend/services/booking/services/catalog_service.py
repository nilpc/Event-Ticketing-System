...
class CatalogService:
    ...
    async def get_seat_map(self, show_id: UUID, section: str | None=None) -> SeatMapResponse:
        cache_key = f'seatmap:{show_id}:{section}' if section else f'seatmap:{show_id}'
        try:
            cached = await self.cache.get(cache_key)
            if cached is not None:
            ...
            logger.warning('cache_read_failed', key=cache_key)
        seats = await self.catalog_repo.get_seat_map(show_id, section)
    ...
    async def get_sections(self, show_id: UUID) -> list[SectionSummaryResponse]:
        from services.booking.schemas.catalog import SectionSummaryResponse
        cache_key = f'sections:{show_id}'
        try:
            cached = await self.cache.get(cache_key)
            if cached is not None:
                return [SectionSummaryResponse.model_validate(s) for s in json.loads(cached)]
        except Exception:
            logger.warning('cache_read_failed', key=cache_key)
        sections = await self.catalog_repo.get_sections(show_id)
        responses = [SectionSummaryResponse.model_validate(s) for s in sections]
        try:
            await self.cache.set(cache_key, json.dumps([r.model_dump(mode='json') for r in responses]), ttl=SEATMAP_CACHE_TTL)
        except Exception:
            logger.warning('cache_write_failed', key=cache_key)
        return responses
