...
async def _seed_show(session: AsyncSession, seat_ids: list[str]) -> uuid.UUID:
    ...
    await session.flush()
    show_id = uuid.uuid4()
    showtime = Showtime(show_id=show_id, event_id=event.event_id, venue_id=venue.venue_id, front_price=Decimal('50.00'), middle_price=Decimal('50.00'), back_price=Decimal('50.00'), start_time=datetime.now(UTC) + timedelta(hours=1), end_time=datetime.now(UTC) + timedelta(hours=3))
