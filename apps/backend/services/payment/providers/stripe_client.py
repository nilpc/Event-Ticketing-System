...
class StripeClient:
    ...
    async def create_payment_intent(
        ...
        try:
            ...
            logger.info(
                "stripe_intent_created", intent_id=intent.id, amount=amount_cents, currency=currency
            )
            return intent
    ...
    def construct_webhook_event(self, payload: bytes, sig_header: str) -> stripe.Event:
        if not settings.STRIPE_WEBHOOK_SECRET:
            raise ValueError(
                "STRIPE_WEBHOOK_SECRET is not configured. Set it in your environment or .env file."
            )
