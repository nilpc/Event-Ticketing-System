"""Multi-seat booking: booking.booking_seats junction + seat cascade FKs.

Revision ID: 005
Revises: 004

Combines the booking_seats junction with the seat-cascade changes so admin
catalog deletion (FR-4) is not blocked by booking history:
- booking_seats (show_id, seat_id) -> seats: CASCADE
- bookings      (show_id, seat_id) -> seats: CASCADE (legacy single-seat FK)
"""
...
def upgrade() -> None:
    ...
    # Legacy bookings.seat_id FK -> seats: RESTRICT -> CASCADE so deleting a
    # seat (e.g. when an admin deletes a showtime/event/venue) is not blocked
    # by single-seat booking history.
    op.drop_constraint(
