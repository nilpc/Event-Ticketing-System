...
async def _make_admin(client: AsyncClient, email: str, password: str) -> str:
    ...
    async with async_session_factory() as session:
        await session.execute(update(User).where(User.user_id == UUID(user_id)).values(is_admin=True))
...
async def test_full_booking_flow(client: AsyncClient) -> None:
    ...
    r = await client.post('/v1/admin/venues', json={'name': 'Integration Test Arena', 'capacity': 10}, headers=admin_h)
    assert r.status_code == 201, f'Create venue failed: {r.text}'
    venue_id = r.json()['venue_id']
    r = await client.post('/v1/admin/events', json={'event_type': 'EVENT', 'name': 'Integration Test Concert'}, headers=admin_h)
    ...
    r = await client.post('/v1/admin/showtimes', json={'event_id': event_id, 'venue_id': venue_id, 'front_price': 75.0, 'middle_price': 75.0, 'back_price': 75.0, 'start_time': start, 'end_time': end, 'auto_seats': True}, headers=admin_h)
    ...
    r = await client.post('/v1/auth/signup', json={'email': user_email, 'password': PASSWORD})
    assert r.status_code == 201
    user_id = r.json()['user_id']
    r = await client.post('/v1/auth/login', json={'email': user_email, 'password': PASSWORD})
    assert r.status_code == 200
    user_token = r.json()['access_token']
    user_h = {'Authorization': f'Bearer {user_token}'}
    r = await client.post('/v1/queue/join', json={'show_id': show_id}, headers=user_h)
    ...
    r = await client.get(f'/v1/queue/status?show_id={show_id}', headers=user_h)
    assert r.status_code == 200
    status_resp = r.json()
    assert status_resp['status'] == 'admitted', f'Not admitted: {status_resp}'
    assert status_resp['queue_token'] == queue_token
    r = await client.post('/v1/seats/lock', json={'show_id': show_id, 'seat_ids': seat_ids}, headers=user_h)
    ...
    r = await client.post('/v1/book', json={'show_id': show_id, 'seat_ids': seat_ids, 'idempotency_key': idempotency_key}, headers={**user_h, 'X-Queue-Token': queue_token})
    ...
    assert str(book_resp['booking_id']) in booking_ids, f"Booking {book_resp['booking_id']} not found in {booking_ids}"
    r = await client.get(f'/v1/showtimes/{show_id}/seats')
    assert r.status_code == 200
    updated_seats = r.json()['seats']
    for s in updated_seats:
        if s['seat_id'] in seat_ids:
            assert s['status'] == 'PENDING_PAYMENT', f"Seat {s['seat_id']} should be PENDING_PAYMENT, got {s['status']}"
        else:
            assert s['status'] == 'AVAILABLE', f"Seat {s['seat_id']} should be AVAILABLE, got {s['status']}"
