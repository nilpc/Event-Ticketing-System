...
VENUES: list[tuple[str, int]] = [('Madison Square Garden', 20789), ('The O2 Arena', 20000), ('Red Rocks Amphitheatre', 9525), ('Hollywood Bowl', 17500), ('Sydney Opera House', 2679), ('Wembley Stadium', 90000), ('Ryman Auditorium', 2362), ('Bridgestone Arena', 19395)]
...
class _EventDef(TypedDict):
    ...
    hours_from_now: int
    duration_hours: int
EVENTS: list[_EventDef] = [{'name': 'Dune: Part Three — World Premiere', 'description': "The epic conclusion to Denis Villeneuve's sci-fi saga. Red carpet premiere with cast Q&A.", 'event_type': 'MOVIE', 'venue_idx': 0, 'base_price': 120.0, 'hours_from_now': 24, 'duration_hours': 3}, {'name': 'Interstellar: 10th Anniversary Screening', 'description': "Christopher Nolan's masterpiece returns to the big screen in IMAX with a live orchestral score.", 'event_type': 'MOVIE', 'venue_idx': 1, 'base_price': 85.0, 'hours_from_now': 48, 'duration_hours': 3}, {'name': 'Beyoncé — Renaissance World Tour', 'description': 'The global superstar performs her chart-topping hits live in a spectacular production.', 'event_type': 'EVENT', 'venue_idx': 5, 'base_price': 150.0, 'hours_from_now': 72, 'duration_hours': 3}, {'name': 'NBA Finals — Game 5', 'description': 'The championship showdown. Can the underdogs force a Game 6?', 'event_type': 'EVENT', 'venue_idx': 7, 'base_price': 95.0, 'hours_from_now': 36, 'duration_hours': 3}, {'name': 'Coldplay — Music of the Spheres Tour', 'description': 'An immersive, sustainable concert experience with stunning visuals and fan-favorite anthems.', 'event_type': 'EVENT', 'venue_idx': 3, 'base_price': 110.0, 'hours_from_now': 96, 'duration_hours': 3}, {'name': 'The Beatles Tribute — Let It Be', 'description': "A multi-award-winning West End show recreating the magic of The Beatles' final years.", 'event_type': 'EVENT', 'venue_idx': 6, 'base_price': 65.0, 'hours_from_now': 12, 'duration_hours': 2}, {'name': 'Marvel Studios: Avengers Secret Wars Premiere', 'description': 'The biggest crossover event in cinema history. First screening with surprise guest appearances.', 'event_type': 'MOVIE', 'venue_idx': 0, 'base_price': 130.0, 'hours_from_now': 168, 'duration_hours': 3}, {'name': 'Formula 1 — Monaco Grand Prix Viewing Party', 'description': "Watch the world's most prestigious race on the big screen with live commentary and food trucks.", 'event_type': 'EVENT', 'venue_idx': 2, 'base_price': 45.0, 'hours_from_now': 60, 'duration_hours': 5}, {'name': 'Hamilton — Broadway Revival', 'description': "Lin-Manuel Miranda's revolutionary musical returns with a star-studded new cast.", 'event_type': 'EVENT', 'venue_idx': 4, 'base_price': 175.0, 'hours_from_now': 120, 'duration_hours': 3}, {'name': 'Stand-Up Comedy Night — Dave Chappelle', 'description': 'An evening of sharp, unfiltered comedy from one of the greatest of all time.', 'event_type': 'EVENT', 'venue_idx': 1, 'base_price': 90.0, 'hours_from_now': 10, 'duration_hours': 2}]
SEATS = [('A1', 'vip', 150.0), ('A2', 'vip', 150.0), ('A3', 'vip', 150.0), ('B1', 'premium', 100.0), ('B2', 'premium', 100.0), ('B3', 'premium', 100.0), ('C1', 'standard', 75.0), ('C2', 'standard', 75.0), ('C3', 'standard', 75.0), ('D1', 'standard', 75.0), ('D2', 'standard', 75.0), ('D3', 'standard', 75.0)]
    ...
    async with factory() as session:
        async with session.begin():
            ...
            for ev in EVENTS:
                ...
                sid = uuid4()
                show_ids.append(sid)
                await session.execute(text("INSERT INTO booking.showtimes (show_id, event_id, venue_id, base_price, start_time, end_time) VALUES (:sid, :eid, :vid, :price, NOW() + :hours * INTERVAL '1 hour', NOW() + (:hours + :dur) * INTERVAL '1 hour')"), {'sid': sid, 'eid': eid, 'vid': vid, 'price': ev['base_price'], 'hours': ev['hours_from_now'], 'dur': ev['duration_hours']})
            for sid in show_ids:
                for seat_id, tier, price in SEATS:
                    await session.execute(text("INSERT INTO booking.seats (show_id, seat_id, tier, price, status) VALUES (:sid, :seat_id, :tier, :price, 'AVAILABLE')"), {'sid': sid, 'seat_id': seat_id, 'tier': tier, 'price': price})
