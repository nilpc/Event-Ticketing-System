...
class BookingRepository:
    ...
    async def create_pending_booking(
        ...
        for seat_id, price in seat_prices:
            self.session.add(
                BookingSeat(booking_id=booking_id, show_id=show_id, seat_id=seat_id, price=price)
            )
    ...
    async def list_bookings_for_user(self, user_id: UUID) -> list[dict]:
        ...
        seats_result = await self.session.execute(
            ...
            .join(
                Seat, (BookingSeat.show_id == Seat.show_id) & (BookingSeat.seat_id == Seat.seat_id)
            )
    ...
    async def get_zombie_bookings(self, cutoff: datetime) -> list[Booking]:
        result = await self.session.execute(
            select(Booking).where(
                Booking.status == BookingStatus.PENDING, Booking.expires_at < cutoff
            )
        )
        return list(result.scalars().all())
    ...
    async def add_outbox_event(
        self, aggregate_type: str, aggregate_id: UUID, event_type: str, payload: dict
    ) -> None:
        event = OutboxEvent(
    ...
    async def log_webhook_event(self, event_id: str, event_type: str, payload: str) -> bool:
        event = ProcessedWebhookEvent(
            event_id=event_id, event_type=event_type, payload={"raw": payload}
        )
        self.session.add(event)
