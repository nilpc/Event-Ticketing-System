"""FR-5: PaymentService tests — fake Stripe provider, real DB (no live Stripe)."""
...
async def _create_payment(
    ...
    """Insert a payment row for a booking (no Stripe call)."""
    ...
    await repo.update_payment_record(
        payment_id=payment_id,
...
async def test_create_intent_happy_path(db_session, booking_fixture) -> None:
    """FR-5: Initiated record written before Stripe call; intent linked on success."""
    provider = FakeStripeClient()
    svc = PaymentService(db_session, provider)

    response = await svc.create_intent(
        booking_id=booking_fixture["booking_id"],
...
async def test_create_intent_booking_not_found(db_session, booking_fixture) -> None:
    """FR-5: Unknown booking_id → NotFoundError, no Stripe call."""
    provider = FakeStripeClient()
    svc = PaymentService(db_session, provider)
...
async def test_create_intent_wrong_owner(db_session, booking_fixture) -> None:
    """FR-5: Booking owned by another user → NotFoundError (no info leak)."""
    provider = FakeStripeClient()
    svc = PaymentService(db_session, provider)
...
async def test_create_intent_expiring_booking(db_session, booking_fixture) -> None:
    """FR-5: Booking within the 2-min expiry guard → BookingConflictError."""
    provider = FakeStripeClient()
    svc = PaymentService(db_session, provider)

    await db_session.execute(
        update(Booking)
        .where(Booking.booking_id == booking_fixture["booking_id"])
...
async def test_create_intent_reuses_existing_intent(db_session, booking_fixture) -> None:
    """FR-5: Non-terminal intent exists → reuse via retrieve, no new Stripe intent."""
    existing_payment_id = await _create_payment(
        db_session, booking_fixture["booking_id"], "pi_existing"
    )
    provider = FakeStripeClient()
    svc = PaymentService(db_session, provider)
...
async def test_create_intent_provider_failure(db_session, booking_fixture) -> None:
    """FR-5: Stripe failure → payment marked FAILED, no orphan intent left."""
    provider = FakeStripeClient()
...
async def test_sync_payment_succeeded(db_session, booking_fixture) -> None:
    """FR-5: Stripe succeeded → payment SUCCEEDED, booking CONFIRMED, seats SOLD, outbox written."""
    ...
    response = await svc.sync_payment(
        payment_id=payment_id,
        user_id=booking_fixture["user_id"],
    )
    ...
    payment = (
        await db_session.execute(select(Payment).where(Payment.payment_id == payment_id))
    ).scalar_one()
    ...
    seats = (
        await db_session.execute(
            select(Seat).where(Seat.show_id == booking_fixture["show_id"])
        )
    ).scalars().all()
    ...
    outbox = (
        await db_session.execute(
            select(OutboxEvent).where(OutboxEvent.aggregate_id == booking_fixture["booking_id"])
        )
    ).scalars().all()
...
async def test_sync_payment_pending(db_session, booking_fixture) -> None:
    """FR-5: Stripe still processing → no state change, booking stays PENDING."""
...
async def test_sync_payment_not_found(db_session, booking_fixture) -> None:
    """FR-5: Unknown payment_id → NotFoundError."""
...
async def test_sync_payment_wrong_owner(db_session, booking_fixture) -> None:
    """FR-5: Payment belonging to another user → NotFoundError."""
