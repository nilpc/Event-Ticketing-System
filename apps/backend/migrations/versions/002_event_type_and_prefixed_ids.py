...
def upgrade() -> None:
    ...
    op.add_column('events', sa.Column('event_type', sa.Enum('MOVIE', 'EVENT', name='event_type', create_type=False), nullable=True), schema='booking')
    op.drop_constraint('showtimes_event_id_fkey', 'showtimes', schema='booking', type_='foreignkey')
    op.add_column('events', sa.Column('event_id_new', sa.String(10), nullable=True), schema='booking')
    ...
    op.execute('ALTER TABLE booking.showtimes ADD CONSTRAINT showtimes_event_id_fkey FOREIGN KEY (event_id) REFERENCES booking.events (event_id_new) ON DELETE CASCADE')
...
def downgrade() -> None:
    op.drop_constraint('showtimes_event_id_fkey', 'showtimes', schema='booking', type_='foreignkey')
    op.add_column('events', sa.Column('event_id_old', sa.dialects.postgresql.UUID(as_uuid=True), nullable=True), schema='booking')
    ...
    op.alter_column('showtimes', 'event_id', schema='booking', type_=sa.dialects.postgresql.UUID(as_uuid=True), nullable=False)
    op.create_foreign_key('showtimes_event_id_fkey', 'showtimes', 'events', ['event_id'], ['event_id_old'], schema='booking', ondelete='CASCADE')
    op.drop_column('events', 'event_id', schema='booking')
