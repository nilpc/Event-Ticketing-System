...
def _showtime_payload(event_id: str, venue_id: str, price: float=50.0) -> dict:
    ...
    return {'event_id': event_id, 'venue_id': venue_id, 'base_price': price, 'start_time': start, 'end_time': end, 'auto_seats': False}
...
async def test_merchant_cannot_modify_showtime_for_others_event(client: AsyncClient) -> None:
    ...
    r = await client.put(f'/v1/admin/showtimes/{show_id}', json={'base_price': 99.0}, headers=_auth(token_b))
...
async def test_merchant_cannot_modify_showtime_for_unowned_event(client: AsyncClient) -> None:
    ...
    r = await client.put(f'/v1/admin/showtimes/{show_id}', json={'base_price': 42.0}, headers=_auth(merchant))
...
async def test_merchant_can_batch_create_showtimes(client: AsyncClient) -> None:
    ...
    r = await client.post('/v1/admin/showtimes/batch', json={'event_id': event_id, 'venue_id': venue_id, 'base_price': 50.0, 'auto_seats': False, 'slots': slots}, headers=_auth(token))
    ...
    r = await client.post('/v1/admin/showtimes/batch', json={'event_id': event_id, 'venue_id': venue_id, 'base_price': 50.0, 'auto_seats': False, 'slots': overlapping}, headers=_auth(token))
...
async def test_batch_create_enforces_ownership(client: AsyncClient) -> None:
    ...
    venue_id = await _create_venue(client, token_a)
    base = datetime.now(UTC)
    r = await client.post('/v1/admin/showtimes/batch', json={'event_id': event_id, 'venue_id': venue_id, 'base_price': 50.0, 'auto_seats': False, 'slots': [{'start_time': (base + timedelta(hours=2)).isoformat(), 'end_time': (base + timedelta(hours=4)).isoformat()}]}, headers=_auth(token_b))
