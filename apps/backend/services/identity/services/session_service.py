...
class SessionService:
    ...
    async def issue_token_pair(self, user_id: UUID) -> LoginResponse:
        ...
        return LoginResponse(access_token=access_token, refresh_token=raw_refresh, is_admin=user.is_admin if user else False, is_master_admin=user.is_master_admin if user else False)
    ...
    async def refresh_access_token(self, refresh_token_raw: str) -> LoginResponse:
        ...
        if not user.is_active or user.deleted_at is not None:
            logger.warning('refresh_token_used_by_inactive_user', user_id=str(user.user_id), is_active=user.is_active, deleted=user.deleted_at is not None)
            raise NotFoundError('Refresh token not found.')
        new_raw, _ = await rotate_refresh_token(old_token_id=stored.token_id, user_id=stored.user_id, session=self.session)
        await self.session.commit()
        access_token = create_access_token(str(stored.user_id))
        return LoginResponse(access_token=access_token, refresh_token=new_raw, is_admin=user.is_admin, is_master_admin=user.is_master_admin)
