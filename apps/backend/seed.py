        ...
        async with session.begin():
            ...
            if master_id is None:
                ...
                if os.getenv("ADMIN_PASSWORD") is None:
                    print(
                        f"[seed] No ADMIN_PASSWORD set — generated admin password: {ADMIN_PASSWORD}"
                    )
                await session.execute(
                    text(
                        "INSERT INTO identity.users (user_id, email, password_hash, is_active, is_admin, is_master_admin) VALUES (:uid, :email, :pw, true, true, true)"
                    ),
                    {
                        "uid": master_id,
                        "email": MASTER_ADMIN_EMAIL,
                ...
                await session.execute(
                    text(
                        "UPDATE identity.users SET is_admin = true, is_master_admin = true WHERE user_id = :uid"
                    ),
                    {"uid": master_id},
                )
            ...
            if merchant_id is None:
                ...
                await session.execute(
                    text(
                        "INSERT INTO identity.users (user_id, email, password_hash, is_active, is_admin, is_master_admin) VALUES (:uid, :email, :pw, true, true, false)"
                    ),
                    {
                        "uid": merchant_id,
                        "email": MERCHANT_ADMIN_EMAIL,
                ...
                await session.execute(
                    text(
                        "UPDATE identity.users SET is_admin = true, is_master_admin = false WHERE user_id = :uid"
                    ),
                    {"uid": merchant_id},
                )
            ...
            if venue_count > 0 and reset:
                ...
                await session.execute(
                    text("ALTER SEQUENCE booking.event_serial_seq RESTART WITH 1")
                )
                await session.execute(
                    text("ALTER SEQUENCE booking.movie_serial_seq RESTART WITH 1")
                )
            venue_ids = []
            for name, capacity in VENUES:
                ...
                await session.execute(
                    text(
                        "INSERT INTO booking.venues (venue_id, name, capacity, created_by) VALUES (:vid, :name, :cap, :created_by)"
                    ),
            ...
            for ev in EVENTS:
                ...
                await session.execute(
                    text(
                        "INSERT INTO booking.events (event_id, event_type, name, description, created_by) VALUES (:eid, :etype, :name, :desc, :created_by)"
                    ),
                    {
                        "eid": eid,
                        "etype": ev["event_type"],
                ...
                await session.execute(
                    text(
                        "INSERT INTO booking.showtimes (show_id, event_id, venue_id, base_price, start_time, end_time) VALUES (:sid, :eid, :vid, :price, NOW() + :hours * INTERVAL '1 hour', NOW() + (:hours + :dur) * INTERVAL '1 hour')"
                    ),
                    {
                        "sid": sid,
                        "eid": eid,
            ...
            for sid in show_ids:
                for seat_id, tier, price in SEATS:
                    await session.execute(
                        text(
                            "INSERT INTO booking.seats (show_id, seat_id, tier, price, status) VALUES (:sid, :seat_id, :tier, :price, 'AVAILABLE')"
                        ),
            ...
            if event_count > 0:
                await session.execute(
                    text("SELECT setval('booking.event_serial_seq', :n)"), {"n": event_count}
                )
            if movie_count > 0:
                await session.execute(
                    text("SELECT setval('booking.movie_serial_seq', :n)"), {"n": movie_count}
                )
