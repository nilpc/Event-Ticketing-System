...
async def signup(
    payload: SignupRequest, session: AsyncSession = Depends(get_db_session)
) -> SignupResponse:
...
@router.get(
    "/google/callback", response_model=LoginResponse, responses={400: {"model": ErrorResponse}}
)
async def google_callback(
    code: str, state: str, request: Request, session: AsyncSession = Depends(get_db_session)
) -> LoginResponse:
