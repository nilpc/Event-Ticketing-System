"""FR-9: WebhookService tests — fake provider, real DB, no live Stripe."""
...
def _webhook_service(session: AsyncSession) -> WebhookService:
    """Build a WebhookService with real repos; lock_repo without Redis is a no-op."""
    return WebhookService(
        session,
        BookingRepository(session),
        SeatRepository(session),
...
def _metadata(booking_fixture: dict) -> dict:
    return {
        "booking_id": str(booking_fixture["booking_id"]),
        "show_id": str(booking_fixture["show_id"]),
        "user_id": str(booking_fixture["user_id"]),
        "seat_ids": ",".join(booking_fixture["seat_ids"]),
    }


async def test_webhook_succeeded(db_session, booking_fixture) -> None:
    """FR-9: payment_intent.succeeded → payment SUCCEEDED, booking CONFIRMED, seats SOLD."""
...
async def test_webhook_payment_failed(db_session, booking_fixture) -> None:
    """FR-9: payment_intent.payment_failed → payment FAILED, booking FAILED, seats AVAILABLE."""
...
async def test_webhook_invalid_signature(db_session, booking_fixture) -> None:
    """FR-9: Bad signature → ValueError (router maps to HTTP 400)."""
    svc = _webhook_service(db_session)
    svc.provider = FakeWebhookProvider(
        event=None, error=ValueError("Invalid webhook signature.")
    )
...
async def test_webhook_missing_booking_id(db_session, booking_fixture) -> None:
    """FR-9: Spoofed event without booking_id → silently dropped, no state change."""
...
async def test_webhook_duplicate_event_is_dropped(db_session, booking_fixture) -> None:
    """FR-9: Same event_id replayed → silently dropped, single idempotency row."""
