...
class WebhookService:
    ...
    def __init__(self, session: AsyncSession, booking_repo: BookingRepository, seat_repo: SeatRepository, lock_repo: LockRepository, payment_repo: PaymentRepository | None=None) -> None:
    ...
    async def process_webhook(self, payload: bytes, signature: str) -> None:
        ...
        try:
            async with self.session.begin_nested():
                try:
                    await self.booking_repo.log_webhook_event(event.id, event.type, payload.decode())
                ...
                if event.type == 'payment_intent.succeeded':
                    ...
                    if booking.status == BookingStatus.FAILED:
                        await self.booking_repo.add_outbox_event(aggregate_type='Payment', aggregate_id=booking_id, event_type='REFUND_REQUIRED', payload={'reason': 'Late webhook on failed booking', 'booking_id': str(booking_id)})
                        ...
                        await self.booking_repo.update_booking_status(booking_id, BookingStatus.CONFIRMED, source='webhook')
                        for seat_id in seat_ids:
                            await self.booking_repo.add_outbox_event(aggregate_type='Booking', aggregate_id=booking_id, event_type='BOOKING_CONFIRMED', payload={'booking_id': str(booking_id), 'show_id': str(show_id), 'seat_id': seat_id})
                        terminal = True
                    ...
                    if booking.status == BookingStatus.PENDING:
                        ...
                        await self.booking_repo.update_booking_status(booking_id, BookingStatus.FAILED, source='webhook')
                        terminal = True
        except IntegrityError:
            return
