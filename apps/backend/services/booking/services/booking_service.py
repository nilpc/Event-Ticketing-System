...
class BookingService:
    ...
    def __init__(self, session: AsyncSession, seat_repo: SeatRepository, booking_repo: BookingRepository, lock_repo: LockRepository, cache_repo: CacheRepository) -> None:
    ...
    async def initialize_checkout(self, show_id: uuid.UUID, seat_ids: list[str], user_id: uuid.UUID, idempotency_key: str, queue_token: str, request_id: str) -> BookResponse:
        if not await self.lock_repo.is_idempotency_key_available(idempotency_key):
            ...
            if existing:
                return BookResponse(booking_id=existing.booking_id, status=existing.status.value, expires_at=existing.expires_at)
        ...
        try:
            async with self.session.begin():
                old_seat_ids, status = await self.booking_repo.cancel_active_booking_for_user_show(user_id, show_id)
                if old_seat_ids:
                ...
                await self.booking_repo.create_pending_booking(booking_id=booking_id, user_id=user_id, show_id=show_id, seat_prices=seat_price_pairs, idempotency_key=idempotency_key, amount=total_amount, expires_at=expires_at, correlation_id=request_id)
                await self.booking_repo.add_outbox_event(aggregate_type='Booking', aggregate_id=booking_id, event_type='BOOKING_INITIALIZED', payload={'booking_id': str(booking_id), 'show_id': str(show_id), 'seat_ids': seat_ids})
            ...
            for seat_id in seat_ids:
                await self.lock_repo.release_seat_lock_safe(show_id, seat_id, user_id)
                await self.lock_repo.release_user_hold_limit(show_id, user_id)
            logger.exception('unexpected_persistence_failure', error=str(e), booking_id=str(booking_id))
        ...
        logger.info('booking_initialized', booking_id=str(booking_id), show_id=str(show_id), seat_ids=seat_ids, user_id=str(user_id))
        return BookResponse(booking_id=booking_id, status=BookingStatus.PENDING.value, expires_at=expires_at)
    ...
    async def mock_confirm_booking(self, booking_id: uuid.UUID) -> MockConfirmResponse:
        ...
        await self.booking_repo.update_booking_status(booking_id, BookingStatus.CONFIRMED, source='mock-confirm')
        ...
        return MockConfirmResponse(booking_id=booking_id, status=BookingStatus.CONFIRMED.value, seat_ids=seat_ids)
    ...
    async def list_user_bookings(self, user_id: uuid.UUID) -> list[BookingListItem]:
        ...
        results = []
        for row in rows:
            seats = [BookingSeatInfo(seat_id=s['seat_id'], tier=s['tier'], price=s['price']) for s in row['seats']]
            results.append(BookingListItem(booking_id=row['booking_id'], status=row['status'], seats=seats, amount=row['amount'], currency=row['currency'], created_at=row['created_at'], show_id=row['show_id'], start_time=row['start_time'], end_time=row['end_time'], event_name=row['event_name'], venue_name=row['venue_name']))
        return results
