...
async def _create_payment(
    ...
    await repo.create_payment_record(
        payment_id=payment_id, booking_id=booking_id, amount=Decimal("100.00"), status=status
    )
    await repo.update_payment_record(
        payment_id=payment_id, provider_payment_id=provider_payment_id, status=status
    )
    await session.flush()
    return payment_id
...
def _real_stripe_event(
    event_type: str, metadata: dict, intent_id: str = "pi_test_123", event_id: str | None = None
) -> stripe.Event:
    payload_obj = {
    ...
    ts = int(time.time())
    sig = "t={},v1={}".format(
        ts, stripe.WebhookSignature._compute_signature(f"{ts}.{payload}", secret)
    )
...
async def test_webhook_succeeded(db_session, booking_fixture) -> None:
    ...
    event = make_stripe_event(
        "payment_intent.succeeded", _metadata(booking_fixture), intent_id=intent_id
    )
    ...
    seats = (
        (await db_session.execute(select(Seat).where(Seat.show_id == booking_fixture["show_id"])))
        .scalars()
        .all()
    )
...
async def test_webhook_payment_failed(db_session, booking_fixture) -> None:
    ...
    await _create_payment(db_session, booking_fixture["booking_id"], intent_id)
    event = make_stripe_event(
        "payment_intent.payment_failed", _metadata(booking_fixture), intent_id=intent_id
    )
    svc = _webhook_service(db_session)
    svc.provider = FakeWebhookProvider(event)
    await svc.process_webhook(b"{}", "t=1,v1=deadbeef")
