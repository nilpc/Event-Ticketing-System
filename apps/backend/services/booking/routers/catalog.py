...
async def list_venues(response: Response, svc: CatalogService=Depends(_get_catalog_service)) -> list[VenueResponse]:
    response.headers['Cache-Control'] = CDN_CACHE_HEADER
    return await svc.list_venues()

@router.get('/events', response_model=list[EventResponse])
async def list_events(response: Response, svc: CatalogService=Depends(_get_catalog_service)) -> list[EventResponse]:
    response.headers['Cache-Control'] = CDN_CACHE_HEADER
    return await svc.list_events()

@router.get('/events/{event_id}/showtimes', response_model=list[ShowtimeResponse])
async def list_showtimes_for_event(event_id: str, response: Response, svc: CatalogService=Depends(_get_catalog_service)) -> list[ShowtimeResponse]:
    response.headers['Cache-Control'] = CDN_CACHE_HEADER
    return await svc.list_showtimes_by_event(event_id)

@router.get('/showtimes', response_model=list[ShowtimeResponse])
async def list_showtimes(response: Response, svc: CatalogService=Depends(_get_catalog_service)) -> list[ShowtimeResponse]:
    response.headers['Cache-Control'] = CDN_CACHE_HEADER
    return await svc.list_showtimes()

@router.get('/showtimes/{show_id}', response_model=ShowtimeResponse)
async def get_showtime(show_id: UUID, response: Response, svc: CatalogService=Depends(_get_catalog_service)) -> ShowtimeResponse:
    response.headers['Cache-Control'] = CDN_CACHE_HEADER
...
@router.get('/showtimes/{show_id}/sections', response_model=list[SectionSummaryResponse])
async def get_sections(show_id: UUID, svc: CatalogService=Depends(_get_catalog_service)) -> list[SectionSummaryResponse]:
    return await svc.get_sections(show_id)

@router.get('/showtimes/{show_id}/seats', response_model=SeatMapResponse)
async def get_seat_map(show_id: UUID, section: str | None=None, svc: CatalogService=Depends(_get_catalog_service)) -> SeatMapResponse:
    return await svc.get_seat_map(show_id, section)
