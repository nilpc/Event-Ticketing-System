...
async def _create_event(client: AsyncClient, token: str, name: str) -> str:
    r = await client.post(
        "/v1/admin/events", json={"event_type": "EVENT", "name": name}, headers=_auth(token)
    )
...
async def _create_venue(
    ...
    r = await client.post(
        "/v1/admin/venues", json={"name": name, "capacity": 10}, headers=_auth(token)
    )
...
async def test_merchant_cannot_modify_others_event(client: AsyncClient) -> None:
    ...
    r = await client.put(
        f"/v1/admin/events/{event_id}", json={"name": "Hijacked"}, headers=_auth(token_b)
    )
    ...
    r = await client.put(
        f"/v1/admin/events/{event_id}", json={"name": "A's Concert v2"}, headers=_auth(token_a)
    )
...
async def test_merchant_cannot_modify_unowned_event(client: AsyncClient) -> None:
    ...
    r = await client.put(
        f"/v1/admin/events/{event_id}", json={"name": "Tampered"}, headers=_auth(token)
    )
...
async def test_master_admin_can_modify_any_event(client: AsyncClient) -> None:
    ...
    r = await client.put(
        f"/v1/admin/events/{event_id}", json={"name": "Master Renamed"}, headers=_auth(master)
    )
    ...
    r = await client.put(
        f"/v1/admin/events/{unowned}", json={"name": "Master Fixes Seed"}, headers=_auth(master)
    )
...
async def test_merchant_cannot_update_or_delete_venue(client: AsyncClient) -> None:
    ...
    r = await client.put(
        f"/v1/admin/venues/{venue_id}", json={"capacity": 500}, headers=_auth(merchant)
    )
    ...
    r = await client.put(
        f"/v1/admin/venues/{venue_id}", json={"capacity": 500}, headers=_auth(master)
    )
...
async def test_merchant_cannot_create_showtime_for_others_event(client: AsyncClient) -> None:
    ...
    r = await client.post(
        "/v1/admin/showtimes", json=_showtime_payload(event_id, venue_b), headers=_auth(token_b)
    )
    assert r.status_code == 403, (
        f"Expected 403 cross-owner showtime create, got {r.status_code}: {r.text}"
    )
    r = await client.post(
        "/v1/admin/showtimes", json=_showtime_payload(event_id, venue_b), headers=_auth(token_a)
    )
    assert r.status_code == 403, (
        f"Expected 403 cross-venue showtime create, got {r.status_code}: {r.text}"
    )
    r = await client.post(
        "/v1/admin/showtimes", json=_showtime_payload(event_id, venue_a), headers=_auth(token_a)
    )
...
async def test_merchant_cannot_modify_showtime_for_others_event(client: AsyncClient) -> None:
    ...
    r = await client.put(
        f"/v1/admin/showtimes/{show_id}", json={"base_price": 99.0}, headers=_auth(token_b)
    )
...
async def test_merchant_cannot_modify_showtime_for_unowned_event(client: AsyncClient) -> None:
    ...
    r = await client.put(
        f"/v1/admin/showtimes/{show_id}", json={"base_price": 42.0}, headers=_auth(merchant)
    )
    assert r.status_code == 403, (
...
async def test_large_venue_auto_seats_generates_all_rows(client: AsyncClient) -> None:
    ...
    r = await client.post(
        "/v1/admin/venues", json={"name": "Big Seat Arena", "capacity": 5000}, headers=_auth(token)
    )
