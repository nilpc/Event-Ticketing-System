        ...
        try:
            async with async_session_factory() as session:
                ...
                async with session.begin():
                    ...
                    await booking_repo.add_outbox_event(aggregate_type='Booking', aggregate_id=booking.booking_id, event_type='BOOKING_FAILED', payload={'booking_id': str(booking.booking_id), 'reason': 'sweeper_zombie_revert'})
            async with async_session_factory() as session:
                lock_repo = LockRepository(session, redis_client=get_redis())
                for seat_id in seat_ids:
                    await lock_repo.release_seat_lock_safe(booking.show_id, seat_id, booking.user_id)
                await lock_repo.release_user_hold_limit(booking.show_id, booking.user_id)
            logger.info('zombie_booking_reverted', booking_id=str(booking.booking_id), show_id=str(booking.show_id))
        except Exception as exc:
            logger.error('sweeper_revert_failed', booking_id=str(booking.booking_id), error=str(exc))
