...
async def _make_admin(client: AsyncClient, email: str, password: str) -> str:
    ...
    async with async_session_factory() as session:
        await session.execute(
            update(User).where(User.user_id == UUID(user_id)).values(is_admin=True)
        )
...
async def test_full_booking_flow(client: AsyncClient) -> None:
    ...
    r = await client.post(
        "/v1/admin/venues", json={"name": "Integration Test Arena", "capacity": 10}, headers=admin_h
    )
    ...
    r = await client.post(
        "/v1/seats/lock", json={"show_id": show_id, "seat_ids": seat_ids}, headers=user_h
    )
    ...
    r = await client.post(
        "/v1/book",
        json={"show_id": show_id, "seat_ids": seat_ids, "idempotency_key": idempotency_key},
